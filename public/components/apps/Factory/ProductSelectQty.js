...
export function ProductSelect ({
                  ...
                  <div class="row">
                      <div class="col-4">
                        ...
                        <div class="input-group">
                          <input class="form-control" type="text" id="${UID}_quantity">
  ...
  function open (bomIdRef) {
    bomId = bomIdRef
    productRef = null
    $(`#${UID}_code`).value = ''
    $(`#${UID}_name`).value = ''
    $(`#${UID}_quantity`).value = ''
    $(`#${UID}_quantity`).setAttribute('disabled', '')
    $(`#${UID}_quantity`).nextElementSibling.textContent = ''
    $(`#${UID}_vendor`).value = ''
  ...
  document.body.addEventListener('click', (event) => {
    if (event.target.classList.contains(`${UID}_select`)) {
      ...
      $(`#${UID}_quantity`).removeAttribute('disabled')
      ...
      `
      $(`#${UID}_quantity`).nextElementSibling.textContent = selectedItem.unitOfMeasureCode || '?'
    ...
    if (event.target.classList.contains(`${UID}_save`)) {
      ...
      const quantity = $(`#${UID}_quantity`).value
