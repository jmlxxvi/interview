...
const logContext = 'MASTERDATA:EQUIPMENT:LIST'

const t1 = DataTable({
  target: '#mdaeqp__table',
  params: ['app/masterdata/equipment', 'list'],
  options: {
    orderCol: 'code',
    orderDir: 'asc',
    rowsPerPage: 10
  ...
  columns: {
    status: {
      ...
      transform: (value, row, _id) => {
        const color = value === 'AVAILABLE' ? '' : 'var(--danger)'
        return `<app-badge color="${color}">${value}</app-badge>`
      }
    },
    code: {
      visible: false,
    ...
    id: {
      ...
      transform: (_value, _row, _id) => {
        return '<a href="masterdata/equipment/edit?equipmentId=[id]" class="router-link button link">Edit</a>'
      },
      header: '&nbsp;',
      sortable: false,
      align: 'center'
...
export async function show (params) {
  ...
  $('#mdaeqp__search').addEventListener('input', (event) => {
