import { log } from '../../../../../js/log.js'
import { $ } from '../../../../../js/dom.js'
import { i18nUnixToDate } from '../../../../../js/i18n.js'
import { eventsListen } from '../../../../../js/events.js'
import { DataTable } from '../../../../../components/Tables/DataTable.js'

const logContext = 'INVENTORY:MOVEMENT:LIST'

const t1 = DataTable({
  target: '#invmov__table',
  params: ['app/inventory/movement', 'list'],
  options: {
    title: 'Inventory Movements',
    action: {
      label: 'New Movement',
      href: '#/inventory/movement/edit'
    },
    orderCol: 'created_at',
    orderDir: 'desc'
  },
  columns: {
    movementType: {
      visible: true,
      header: 'Type',
      align: 'center',
      transform: (value, _row, _id) => {
        const colors = {
          RECEIPT: 'var(--success)',
          TRANSFER: 'var(--info)',
          ISSUE: 'var(--warning)',
          RETURN: 'var(--primary)',
          ADJUSTMENT: 'var(--secondary)'
        }
        return `<span style="padding: 0.25rem 0.5rem; border-radius: 0.25rem; background-color: ${colors[value] || 'var(--gray-300)'}; color: white; font-size: 0.875rem;">${value}</span>`
      }
    },
    productName: {
      visible: true,
      header: 'Product',
      align: 'left',
      transform: (value, row, _id) => {
        return value ? `<div style="display: flex; flex-direction: column; align-items: start"><div><strong>${value}</strong></div><div style="font-size: .8rem; color: var(--gray-700)">${row.productCode || ''}</div></div>` : '-'
      }
    },
    quantity: {
      visible: true,
      header: 'Quantity',
      align: 'right',
      transform: (value, row, _id) => {
        return parseFloat(value).toFixed(3) + ' ' + (row.unitOfMeasureCode || '')
      }
    },
    sourceLocationName: {
      visible: true,
      header: 'From',
      align: 'left',
      transform: (value, row, _id) => {
        return value ? `<div style="font-size: .9rem">${value}</div>` : '-'
      }
    },
    destinationLocationName: {
      visible: true,
      header: 'To',
      align: 'left',
      transform: (value, row, _id) => {
        return value ? `<div style="font-size: .9rem">${value}</div>` : '-'
      }
    },
    workOrderCode: {
      visible: true,
      header: 'Work Order',
      align: 'center'
    },
    reason: {
      visible: true,
      header: 'Reason',
      align: 'left',
      transform: (value, _row, _id) => {
        return value ? `<div style="font-size: .9rem; max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${value}</div>` : '-'
      }
    },
    createdBy: {
      visible: true,
      header: 'Created by',
      transform: (value, row, _id) => {
        return `<div style="display: flex; flex-direction: column; align-items: end"><div>${value}</div><div style="font-size: .8rem">${i18nUnixToDate(row.createdAt)}</div></div>`
      },
      align: 'right'
    },
    id: {
      visible: true,
      transform: (_value, _row, _id) => {
        return '<a href="inventory/movement/edit?movementId=[id]" class="router-link button link">View</a>'
      },
      header: '&nbsp;',
      sortable: false,
      align: 'center'
    }
  }
})

// Reload table when data changes
eventsListen('event-table-data-changed', (event) => {
  if (event.table === 'inventory_movement') {
    t1.reload()
  }
})

export async function show (params) {
  log('View show', logContext)

  if (params.reload === 'true') {
    t1.reload()
  }

  $('#invmov__search').addEventListener('input', (event) => {
    t1.search(event.target.value)
  })
}

export async function hide () {
  log('View hide', logContext)
}
